...
class PtData(Dataset):
    def __init__(self,  dim=1):
        data_dir = r"Data"  # r"./organized_results/train_output"
        ...
        for printer in res_dict.keys():
            ...
            for speed in res_dict[printer].keys():
                ...
                addy = np.sqrt(res_dict[printer][speed]['data'][:, 1]**2 +res_dict[printer][speed]['data'][:, 2]**2)
                # train-test split
                if not speed == 30:
            ...
            self.data.append((torch.tensor(y).unsqueeze(1).to(torch.float32), torch.tensor(x/scale).unsqueeze(1).to(torch.float32)))
            self.data.append((torch.tensor(y_test).unsqueeze(1).to(torch.float32), torch.tensor(x_test/scale).unsqueeze(1).to(torch.float32)))
...
class Decoder_kernel(nn.Module):
    def __init__(self, h_dim=128, z_dim=1, device='cuda:0'):
        ...
        #self.centers = torch.rand(h_dim, requires_grad=False)
        #self.stds = torch.ones(h_dim, requires_grad=False)
        ...
        self.fc25 = nn.Linear(h_dim, z_dim)
        '''
        middle_dim = 1024
    ...
    def forward(self, x):
        ...
        xmat = torch.transpose(torch.stack([x for i in range(self.h_dim)])[:,:,0],0,1) #torch.transpose(torch.tile(x, (self.h_dim)), 0, 1)
        #print(x.size())
        #print(xmat.size())
        #print(mumat.size())
        #assert False
        ...
        #sy = (self.fc2(Phi)+self.fc21(Phi)+self.fc22(Phi)+self.fc23(Phi)+self.fc24(Phi))/3.
        sy = self.fcn2(self.fc2n(Phi))
        return sy #+ torch.mean(Phi, dim=1)
    ...
    for epoch in range(epochs):
        ...
        for i in range(len(dataset)//2):
            ...
            if args['fed'] in {'Ditto', 'indiv'}:
                ...
                optimizer_l = torch.optim.Adam(list(decoders[i].parameters())+list(encoders[i].parameters()), lr=learning_rate, weight_decay=wd)
            ...
            for j in range(inner_epoch):
                ...
                if not args['fed'] == 'indiv':
                    ...
                    if args['fed'] == 'Ditto':
                        clientencoder[i].load_state_dict(mo.scalar_mul(coeffs, [encoder, clientencoder[i]]).state_dict())


            #centers[i] = ci.clone().detach()
        decoder.load_state_dict(mo.scalar_mul(torch.ones(len(encoders)) / len(encoders), clientdecoder).state_dict())
        if args['fed'] == 'Ditto':
            encoder.load_state_dict(mo.scalar_mul(torch.ones(len(encoders)) / len(encoders),clientencoder).state_dict())
        ...
        tl = test_loss(dataset, encoder, encoders, decoder, decoders, n_task, device, args, test=0)
...
if __name__ == "__main__":
    ...
    output_dir = 'ptoutputs/' #+ args['dataset'] + '_'
